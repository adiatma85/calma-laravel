    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.journeys.update", [$journey->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $journey->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $journey->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mood_tracker') ? 'is-invalid' : '' }}" name="mood_tracker_id" id="mood_tracker_id">
                    @foreach($mood_trackers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('mood_tracker_id') ? old('mood_tracker_id') : $journey->mood_tracker->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('playlist') ? 'is-invalid' : '' }}" name="playlist_id" id="playlist_id">
                    @foreach($playlists as $id => $entry)
                        <option value="{{ $id }}" {{ (old('playlist_id') ? old('playlist_id') : $journey->playlist->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
