...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.journey.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.journeys.update", [$journey->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.journey.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $journey->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('author') ? 'is-invalid' : '' }}" type="text" name="author" id="author" value="{{ old('author', $journey->author) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $journey->description) !!}</textarea>
            ...
            <div class="table table-bordered table-striped">
              ...
              @foreach ($journey->items as $items)
                  {{-- Question item goes here --}}
                  <div class="form-group col-md-12" id="existing-field-{{$items->id}}">
                    <div class="row">
                      <div class="col-md 11">
                        <input type="text" name="existingQuestion[]" id="existingQuestion-{{$items->id}}" class="form-control" placeholder="Pertanyaan" value="{{ $items->name ?? '' }}">
                      </div>
                      <div class="col-md-1">
                        <button class="btn btn-warning btn-block btn-existing-question" id="removeExistingSign-{{$items->id ?? ''}}">
...
<script>
  ...
  $(document).ready(function() {
          ...
          function appendHiddenElement(numerical){
            $org = 
              `<input type='hidden' name='removed_items[]' value='${numerical}'>`;
              return $org;
          }
      });
  </script>
