...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.moodTrackerReason.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.mood-tracker-reasons.update", [$moodTrackerReason->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mood_tracker') ? 'is-invalid' : '' }}" name="mood_tracker_id" id="mood_tracker_id">
                    @foreach($mood_trackers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('mood_tracker_id') ? old('mood_tracker_id') : $moodTrackerReason->mood_tracker->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reason') ? 'is-invalid' : '' }}" type="text" name="reason" id="reason" value="{{ old('reason', $moodTrackerReason->reason) }}">
